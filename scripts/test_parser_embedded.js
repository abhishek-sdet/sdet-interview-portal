...
const parseSimpleQuestions = (text) => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (sectionRegex.test(line) || line.includes('MISCELLANEOUS')) {
            ...
            console.log(`[DEBUG] Section Header Found: ${line} -> Raw: ${rawName}`);
            ...
            // Important: If matched, skip processing this line as question text
        ...
        if (ansMatch && currentQuestion) {
            ...
            console.log(`[DEBUG] Answer for Q${questions.length + 1} found: ${ansMatch[1]}`);
            continue;
        }
        ...
        if (optMatch && currentQuestion) {
            ...
            if (map[letter] !== undefined) {
                ...
                // console.log(`[DEBUG] Option ${letter} found for Q${questions.length + 1}`);
            }
            continue;
        }
        ...
        if (qMatch) {
            // Log matching for debug
            console.log(`[DEBUG] Potential Question Match: "${line}" matched regex.`);
            ...
            if (currentQuestion && currentQuestion.options.some(o => o)) {
                ...
                console.log(`[FAILURE] Dropping incomplete question: "${currentQuestion.question_text}"`);
            }
...
try {
    ...
    console.log(`\n================================`);
    console.log(`Total Parsed Questions: ${results.length}`);
    console.log(`================================`);

    // Output only summary to avoid overflow, or limited info
    const sectionCounts = {};
    results.forEach((q, i) => {
        const sec = q.section + (q.subsection ? ` (${q.subsection})` : '');
        sectionCounts[sec] = (sectionCounts[sec] || 0) + 1;
        // console.log(`${i+1}. [${sec}] ${q.question_text.substring(0, 30).replace(/\n/g, ' ')}...`);
    });
