...
// ---------------------------------------------------------
//  EXACT COPY OF LOGIC FROM questionParser.js
// ---------------------------------------------------------
...
function detectSections(text) {
    // Preprocess: Add newlines before options if missing
    text = text.replace(/([^\n])\s*([A-Da-d][\.)\]])\s+/g, '$1\n$2 ');
    // Preprocess: Handle "Answer:" validation
...
function parseQuestionsInSection(section) {
    ...
    const saveCurrentQuestion = () => {
        if (currentQuestion && currentOptions.length >= 2) {
            ...
            // Re-detect per question (e.g. Java keywords)
...
// ---------------------------------------------------------
try {
    const filePath = path.join(__dirname, '..', 'Set_B_Fresher_Fixed.txt');
    ...
    // Detailed Check
    allQ.forEach((q, idx) => {
        console.log(`Q${idx + 1} [${q.subsection}]: ${q.question_text.substring(0, 30)}...`);
    });
